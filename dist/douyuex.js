...
const DANMAKU_VERIFY_TRUTH_CACHE_MAX = 2e3;
...
function initPkg_Shield_DanmakuVerify() {
    ...
    shieldTool.insertAdjacentHTML("afterbegin", `<div class="FilterSwitchStatus" id="ex-danmakuVerify">
    <h3>弹幕鉴真</h3>
    <div>
  ...
  <p class="FilterKeywords-intelligentText" style="display: flex; align-items: center; justify-content: space-between;">
    ...
    historySizeInput.addEventListener("input", () => {
        ...
        if (isNaN(value) || value < 50) {
            value = 50;
            historySizeInput.value = 50;
        } else if (value > 2e3) {
            value = 2e3;
            historySizeInput.value = 2e3;
        }
    ...
    ttlInput.addEventListener("input", () => {
        ...
        danmakuVerifyTruthCacheTTL = 60 * value;
        setLocalDanmakuVerifyTruthCacheTTL(60 * value);
    });
    dom.addEventListener("click", () => {
...
function danmakuVerifyAddToHistory(text) {
    ...
    if (danmakuVerifyHistoryCount < danmakuVerifyHistorySize) danmakuVerifyHistoryCount++;
}

function danmakuVerifySearchHistory(quotedText) {
    const normalized = dvNormalize(quotedText);
    for (let i = 0; i < danmakuVerifyHistoryCount; i++) if (danmakuVerifyHistory[i] && danmakuVerifyHistory[i].includes(normalized)) return true;
    return false;
}

function danmakuVerifyAddTruthCache(text) {
    const normalized = dvNormalize(text);
    if (danmakuVerifyTruthCache.has(normalized)) danmakuVerifyTruthCache.delete(normalized); else if (danmakuVerifyTruthCache.size >= DANMAKU_VERIFY_TRUTH_CACHE_MAX) danmakuVerifyTruthCache.delete(danmakuVerifyTruthCache.keys().next().value);
    danmakuVerifyTruthCache.set(normalized, Date.now() + 1e3 * danmakuVerifyTruthCacheTTL);
}
...
function danmakuVerifyCleanupTruthCache() {
    const now = Date.now();
    for (var [ key, expiry ] of danmakuVerifyTruthCache.entries()) if (expiry <= now) danmakuVerifyTruthCache.delete(key); else break;
}
...
function danmakuVerifyCheck(text) {
    ...
    if (danmakuVerifyCheckTruthCache(quotedContent)) return true;
...
function startDanmakuVerify() {
    if (danmakuVerifyDomHook) return;
    StyleHook_set("Ex_Style_DanmakuVerify", `.ex-dv-badge {
      display: inline-block;
      margin-left: 4px;
      padding: 0 3px;
      font-size: 10px;
    ...
    const setupHook = () => {
        ...
        danmakuVerifyDomHook = new DomHook("#js-barrage-list", false, m => {
            ...
            if (!contentDoms || 0 === contentDoms.length) return;
            const contentDom = contentDoms[0];
            const text = contentDom.innerText ? contentDom.innerText.trim() : "";
            if (!text) return;
            if (danmakuVerifyCheck(text)) danmakuVerifyMarkTrue(contentDom);
            danmakuVerifyAddToHistory(text);
        });
    };
    if (document.getElementById("js-barrage-list")) setupHook(); else {
        let timer = setInterval(() => {
...
function startDanmakuVerifyCleanupTimer() {
    ...
    danmakuVerifyCleanupTimer = setInterval(danmakuVerifyCleanupTruthCache, 3e4);
}
...
function getLocalIsDanmakuVerify() {
    return "1" === localStorage.getItem("ExSave_isDanmakuVerify");
}
...
function getLocalDanmakuVerifyHistorySize() {
    ...
    if (saved) {
        ...
        if (!isNaN(value) && value >= 50 && value <= 2e3) return value;
    }
    return 500;
}
